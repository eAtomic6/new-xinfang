...
export default {
  ...
  data() {
    return {
      ...
      isUplus:"3" ,        // 用来判断是否是u+客源列表
      loading:true,
      allCompanyData:[],     //公客列表的所有公司数据
    };
  },
  created() {
    this.$axios.get("/api/customer/public/list/can").then(res => {
      res = res.data
      if(res.status === 200){
        this.getDictionary();
        this.getAllCompany()
  ...
  methods: {
    //   获取所有公司接口
      getAllCompany(){
          this.$axios.get("/api/department/company/list",{params:{dataAuth:true}}).then(res => {
              res = res.data
              if(res.status === 200){
    ...
    getTableData(data, int) {
      ...
      this.$axios
        .get("/api/customer/public/list", { params: this.keywordObj })
        .then(res => {
          res = res.data;
          //  debugger
          if (res.status === 200) {
