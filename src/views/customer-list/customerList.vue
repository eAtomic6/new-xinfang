...
import addCustomer from "./addUser/addCustomer"
export default {
  components: {
    customerQuery,
    customerTable,
  ...
  data() {
    return {
      ...
      loading:true,
      addCustomerBox:false,  //新增客户弹框显示隐藏
      // customerId:""         // 激活弹框中的客户id存储
    };
  },
  created() {
    this.$axios.get("/api/customer/list/can").then(res => {
      res = res.data
      if(res.status === 200){
        this.getDictionary();
      }
  ...
  methods: {
    ...
    getTableData(data, int) {
      ...
      this.$axios
        .get("/api/customer/list", { params: this.keywordObj })
        .then(res => {
          res = res.data;
          //  debugger
          if (res.status === 200) {
