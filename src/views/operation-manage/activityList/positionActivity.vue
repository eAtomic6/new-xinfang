...
<template>
  <div>
      ...
      <el-dialog :title="positionDialogTitle" :visible.sync="addAcivityDialog" @close="clearAddDiaolog" width="760px">
            <add-position ref="addChild" v-if="addAcivityDialog" :editData="editData"  @closeAddDialog="closeAddDialog" :companyData="companyData"></add-position>
        </el-dialog>
...
import {showTableData} from './positionActivityData.js'
export default {
    components:{queryCom,tableCom,addPosition,getList},
  data () {
    return {
  ...
  methods: {
      addNewActivity(){
          ...
          this.editData = {}
          this.positionDialogTitle="新增固定活动"
      },
    //   获取所有公司数据
      getCompanyData(){
    ...
    //   获取table数据
      getTableData(data){
        ...
        //   console.log(this.keywordData);
          this.$axios.get("/api/queryFixedActivityList",{params:this.keywordData}).then(res => {
              res = res.data
              if(res.status === 200){
    ...
    editActivity(data){
        ...
        this.addAcivityDialog = true
        this.editData = data
        this.positionDialogTitle = "编辑固定活动"
    },
    ...
    sureDelete(data){
        let qs = require("qs")
        this.$axios.put("/api/deleteFixedActivities", qs.stringify(this.params)).then(res => {
            res = res.data
            // debugger
            if(res.status === 200){
